...
def find_flows(pcap_to_read, number_of_packets):
	...
	create_tmp_csv = "tshark -r " + pcap_to_read + " -T fields -e ipv6.src -e ipv6.dst -E header=y -E separator=, > tmp.csv"
	process = subprocess.Popen(create_tmp_csv, shell=True, stdout=subprocess.PIPE)
	process.wait()
	#Count of packets that compose each flow, grouping by src and dst (fl is wrong: consider the case 
	#where fl is injected!)
	df = pd.read_csv('tmp.csv')
	df = df.groupby(['ipv6.src', 'ipv6.dst']).size().to_frame('#pkts').reset_index()
	...
	return df.loc[df['#pkts'] >= number_of_packets]

def extract(pcap, source, destination, targeted_field, number_of_packets):
	...
	for x in range(len(pkts)):
		if secret_index < number_of_packets:
			if source == pkts[x][IPv6].src and destination == pkts[x][IPv6].dst:
	...
	while True:
		...
		if operation.strip().isdigit():
			...
			if not what_flow in flows.index:
				...
				source, destination = flow_selection(flows, what_flow)
				break
		else:
			print("This operation is not supported!")
	print('-' * 25)
	print("Conversation: src = " + str(source) + " dst = " + str(destination))
	print(extract(settings.pcap, source, destination, settings.field, settings.packets))
else:
